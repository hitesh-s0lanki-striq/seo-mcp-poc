...
# Service account credentials - can be provided as JSON content via GSC_CREDENTIALS env var
# Falls back to looking in script directory and current working directory
GSC_CREDENTIALS = os.environ.get("GSC_CREDENTIALS", "")
...
def get_gsc_service():
    ...
    # Try service account authentication from GSC_CREDENTIALS env var first
    if GSC_CREDENTIALS:
        try:
            creds_info = json.loads(GSC_CREDENTIALS)
            ...
            raise ValueError(
                f"GSC_CREDENTIALS environment variable contains invalid JSON: {str(e)}"
            )
        except Exception as e:
            raise ValueError(
                f"Failed to load credentials from GSC_CREDENTIALS environment variable: {str(e)}"
            )
    
    # Try service account authentication from file paths
    ...
    raise FileNotFoundError(
        ...
        f"2. Set the GSC_CREDENTIALS environment variable with the JSON content of your service account credentials, or\n"
        f"3. Place a service account credentials file in one of these locations: "
        f"{', '.join([p for p in POSSIBLE_CREDENTIAL_PATHS if p])}"
    )
